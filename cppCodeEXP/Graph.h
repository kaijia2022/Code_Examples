...
class Graph {
    public:
        /**
         * member variable: an adjacency list representation of the graph;
         * key: vertex, a struct containing the name of current vertex, its previous and its dist;
         * value: a vector of tuple<int, string,int> (weight, endpoint, 0 if not duplicate, 1 if duplicate)
         */
        unordered_map<string, vector<tuple<double,string,int>>> graph;
        /**
        ...
        /* clear the information tuple of each edgelist */
        void clear_info();
...
class TupleCompare {
    public:
       bool operator()(tuple<string,double> a, tuple<string,double> b);
};
